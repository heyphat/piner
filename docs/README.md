...
| [tradingview-strategy-report-metrics.md](./tradingview-strategy-report-metrics.md) | **TradingView's Strategy Report metrics, one by one**: definition + formula for all 53 official metrics, each cross-referenced against its piner field. 44/53 implemented; the rest flagged as not modeled (margin/liquidation) or unpublished-formula (account size required, return of max drawdown). |
| [strategy-engine-math.md](./strategy-engine-math.md)                               | **The complete math & running logic of the strategy engine**, formula-first with `file:line` citations: execution model, config/sizing, order lifecycle & the fill engine (trigger inequalities, fill-price selection), commission/slippage/tick math, the margin & `Account` model (funds gate, forced liquidation), position/closed-trade accounting, equity/drawdown/run-up curves, the `StrategyReport`, `strategy.risk.*`, derived metrics, and portfolio aggregation.                          |
...
- **Two backends, byte-for-byte identical.** `compile(src)` emits both a JS closure
  ...
  same runtime `$`; the whole test suite asserts they agree. **682 tests** (1 skipped
