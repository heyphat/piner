...
| Feature                                                                                      | Status | Location                                                               | Notes                                                                                                                                                                                                                                                                                                                                                                                              |
| -------------------------------------------------------------------------------------------- | ------ | ---------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `strategy(title, …)` config                                                                  | ⚠️     | `strategy.ts` (`configure`); `context.ts` (`configureStrategy`)        | Core settings are supported. `use_bar_magnifier` is **experimental/partial**: validated injected LTF data affects historical fills when `calc_on_order_fills=false`; no-data/empty/cap buckets fall back per bar. COOF and realtime keep the established chart-OHLC path and report magnifier inactive/fallback. PineForge provides differential proxy evidence only—not TradingView ground truth. |
...
### Bar Magnifier support boundary

Supported publicly: historical, validated `BarMagnifierData`,
`calc_on_order_fills=false`, independent chronological LTF OHLC rows, LTF fill
timestamps, chart OHLC/time restoration before Pine/POC, 200,000-row capping,
and explicit per-bar fallback/reporting. The JS and interpreter backends, full
run and step execution, and isolated portfolio sleeves share this path.
Shared-account portfolios remain outside this experimental slice and reject
injected Bar Magnifier data rather than executing an unvalidated cross-sleeve
intrabar/account schedule.

Not claimed as TradingView parity: COOF magnifier recalculation, realtime
magnification, same-chart entry-plus-bracket semantics, exact risk/margin/tie
cadence, and provider equality with a TradingView feed. PineForge black-box
A/B/E results validate already-live bracket/limit/stop ordering as a practical
second-engine oracle; its known flat-position exit-lifetime and gap-fill-price
divergences are excluded. See `dev-docs/bar-magnifier-plan.md` §5.5 and §10.
