...
a *cross-check*, not an oracle — it has its own bugs. Where the two disagree, the
...
| **(b) Parity coverage of the *parity-able* universe** | **134** / ~310 | **~43%** |
| (c1) piner *implementation* coverage (for contrast) | 846 / 884 | **95.7%** |
...
- **What covers the rest.** PineTS is the targeted *external* sanity check on
...
- **No live data/infra:** session/timezone *filtering* in `time()`/`time_close()`;
