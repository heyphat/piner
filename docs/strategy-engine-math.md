...
`qtyFor(order, price)` (`strategy.ts:560-563`) uses `order.qty` when it is a valid (non-NaN) number, else falls back to `defaultQty(price)`.

`defaultQty(price)` (`strategy.ts:566-571`):
...
| ------------------- | ----------------------------------------- | ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `fixed`             | `qty = qtyValue`                          | `strategy.ts:570` | Raw contract count, price-independent.                                                                                                                    |
| `cash`              | `qty = qtyValue / price`                  | `strategy.ts:568` | Spend `qtyValue` currency units of notional at the fill price.                                                                                            |
| `percent_of_equity` | `qty = (qtyValue / 100) * equity / price` | `strategy.ts:569` | Allocate `qtyValue`% of live account equity as notional. `equity` is the live getter `account.equity` (`strategy.ts:433-435`), re-evaluated at call time. |

**Rounding: none.** `defaultQty()` returns fractional contracts and `openOrAdd()` (`strategy.ts:1278-1320`) consumes them unrounded. The only `Math.round`/`Math.trunc` in the file are the margin-liquidation-price tick round (`strategy.ts:478`) and a trade-index truncation (`strategy.ts:767`) — neither touches order quantity. This is a deliberate deviation from TradingView, which floors cash/percent-derived quantities to whole contracts (§16).

Exposed to scripts as `strategy.default_entry_qty(price)` (`strategy.ts:1515`), defaulting `price` to `b.host.close`.

---

## 4. Order lifecycle

All entry verbs are methods on `StrategyBroker`. They do **not** fill — they mutate two in-memory queues, `private pending: Order[]` (`strategy.ts:331`) and `private exits: ExitBracket[]` (`strategy.ts:332`); fills are deferred to the driver's per-bar passes (§5). Every broker method early-returns when inactive or halted: `if (!this.active || this.riskHaltActive) return;` (e.g. `strategy.ts:583`).

Two wrapper helpers front every public verb (`strategy.ts:1432+`):

- `gated(when, fn)` (`strategy.ts:1432-1434`): `if (when !== false) fn();` — suppresses only when `when` is **exactly** `false` (na/undefined still fire).
- `opt(x)` (`strategy.ts:34-35`): `x === undefined || isNa(x) ? undefined : Number(x)` — coerces na/undefined numeric args to "not set."

**Exposed constant enums** (`makeStrategyNs`, `strategy.ts:1436-1447`): scripts read `strategy.long`/`strategy.short` (= `DIR_LONG`/`DIR_SHORT`), the sizing enums `strategy.fixed`/`cash`/`percent_of_equity`, `strategy.commission.{percent,cash_per_contract,cash_per_order}`, `strategy.direction.{all,long,short}`, and `strategy.oca.{none,cancel,reduce}` (`strategy.ts:1446`). The `oca.*` constants **exist but OCA is not implemented** — `entry`/`order`/`exit` accept no `oca_name`/`oca_type` argument (signatures `strategy.ts:1464-1509` take only `id,dir,qty,limit,stop,when` plus the exit-bracket args), and `comment`/`alert`/`alert_message` are not modeled either; see §16.
...
Both submit through `submit(o)` (`strategy.ts:575-581`), keyed by id: re-submitting an unfilled `entry`/`order` **replaces in place** rather than stacking (`findIndex(p.id===o.id && kind∈{entry,order})` → replace, else push).

- `entry(id, dir, qty?, limit?, stop?)` (`strategy.ts:582-592`) pushes `{...orderTrigger(limit,stop), kind:'entry', seq:++this.orderSeq}`.
- `order(id, dir, qty?, limit?, stop?)` (`strategy.ts:593-603`) is identical except `kind:'order'`.

The behavioral difference lives in `execute(o, price)` at fill time (`strategy.ts:1227-1275`). On an **opposite-direction** position (`size !== 0 && sign(size) !== dir`, `strategy.ts:1249`):

- `entry` → **reverses**: `closePosition(price)` closes fully flat, then falls through (no `return`) to open the full new qty (`strategy.ts:1250-1251`, `1225-1231`).
- `order` → **nets** (`strategy.ts:1253-1260`): `closable = min(qty, |size|)`, close that much; if `rem = qty - closable > 0`, `openOrAdd(o, dir, rem, price)` opens the crossing remainder as a flip.

On a **same-direction** position, `entry` is capped by pyramiding (below); `order` is uncapped (matches TradingView).

`this.orderSeq` (`strategy.ts:334`) is a monotonic submission counter; its value is stored on the Order as `seq`, then inherited by the resulting Lot as `orderSeq` (`strategy.ts:1303`) for exit-bracket scoping.

### Pyramiding

Enforced at `strategy.ts:1262-1267`: a same-direction `strategy.entry` add is blocked when `openEntryCmdCount >= settings.pyramiding` (`return`). `openEntryCmdCount` (`strategy.ts:296-300`) counts open lots with `entryCmd === true`; `entryCmd` is set only for `strategy.entry` fills (`o.kind === 'entry'`, `strategy.ts:1304`). So the cap counts **open** entry-command lots, not cumulative adds — a close frees capacity. A reversal (opposite dir) always closes-then-opens regardless of the count.

### close / close_all

- `close(id, qty?)` (`strategy.ts:604-611`): gated on an OPEN lot with this id at call time — `if (!this.entryLots.some(lt => lt.id === id)) return;` — so a same-bar `entry(id)`+`close(id)` doesn't round-trip. Pushes `{id, dir:0, qty, kind:'close', otype:'market'}`.
- `close_all()` (`strategy.ts:612-619`): no-op when flat, else pushes `{id:'', dir:0, kind:'closeAll', otype:'market'}`.
...
`exit(id, fromEntry?, qty?, profit?, loss?, stop?, limit?, trailPrice?, trailPoints?, trailOffset?)` (`strategy.ts:620-654`) builds an `ExitBracket`, keyed by id: re-submitting the same id **updates in place while preserving the trailing ratchet** — `if (i>=0) { bracket.trailStop = this.exits[i].trailStop; this.exits[i] = bracket; } else this.exits.push(bracket);` (`strategy.ts:649-653`).

Key fields (`strategy.ts:85-102`): `profit`/`loss` in **ticks** (per-lot, from that lot's fill price), `stop`/`limit` absolute **prices**, `trailPrice` activation price, `trailPoints` activation distance (ticks), `trailOffset` trail distance (ticks), `trailStop` current ratcheting level (NaN until armed), `filled` contracts closed so far, and `maxSeq = this.orderSeq` — the **call-time scope**: only lots whose `orderSeq <= maxSeq` are eligible (`strategy.ts:647`). Bracket price computation and the trailing ratchet are done at fill time (`processExits`/`trailFill`; formulas in §5). Exit brackets are cleared entirely once the position goes flat (`strategy.ts:1174`).

**OCA-style reservation** (`strategy.ts:1066-1094`): eligibility is `(!ex.fromEntry || lt.id === ex.fromEntry) && lt.orderSeq <= ex.maxSeq` (`strategy.ts:1069-1070`). Quantity is reserved **in call order**: an `unreserved` map starts at each lot's qty; iterating brackets in order, each takes only what earlier brackets left of its eligible lots — `want = ex.qty!=null ? max(0, ex.qty - filled) : Infinity`, `take = min(unreserved, want)` (`strategy.ts:1081-1092`). This reproduces TV's reversed-exit demo (qty-19 limit + qty-20 stop on 20 shares → the stop covers exactly 1). A bracket is `spent` once `filled>0 && (no eligible lots left || filled >= qty - 1e-9)` (`strategy.ts:1170-1171`). This is _within-bracket_ reservation only; there is no cross-order OCA-group cancellation (see §16).

### risk.max_position_size (clamp, not block)

`strategy.risk.max_position_size` _reduces_ an entry's qty rather than blocking it: `qty = Math.min(qty, this.riskMaxPositionSize - Math.abs(this.size))` (`strategy.ts:1271-1272`); if `qty <= 0` the entry isn't placed. Applies only to `kind === 'entry'`.

### cancel / cancel_all

- `cancel(id)` (`strategy.ts:847-849`): `this.pending = this.pending.filter(o => o.id !== id || o.otype === 'market')`; `this.exits = this.exits.filter(e => e.id !== id)`. **Market orders are not cancelable.**
- `cancel_all()` (`strategy.ts:851-854`): `pending = pending.filter(o => o.otype === 'market'); exits = []`.

---

## 5. Fill engine

Slippage is computed once per pass: `const slip = this.settings.slippage * this.host.mintick;` (`strategy.ts:882`; exit path `mt = host.mintick`, `strategy.ts:1053-1054`).

### The two passes

`onBar()` (`strategy.ts:858-865`) treats the whole bar as one tick: `this.processTick(open, high, low, open)` (`strategy.ts:862`) — range `[low, high]`, **market fill price = `open`** (next-bar-open execution). Then `barsProcessed++`, and `if (size !== 0) barsInMarket++`.

`onBarClose()` (`strategy.ts:873-877`) — a no-op unless `settings.processOrdersOnClose`. When set, runs `this.processTick(close, close, close, close)` (`strategy.ts:876`) AFTER `main($)` with `o=h=l=marketPx=close`: market orders created this same bar fill at the **close**; limit/stop/exit are tested against the **close only** (the collapsed range never sees the bar's earlier high/low, which predate those orders).

### `processTick(o, h, l, marketPx)` (`strategy.ts:881-936`) — four ordered phases

**Phase 1 — pending orders, in submission order** (`strategy.ts:885-919`); survivors go to `stillPending`:

| Order     | Trigger (long / short)                                                 | Fill price                                                                 | Code                  |
| --------- | ---------------------------------------------------------------------- | -------------------------------------------------------------------------- | --------------------- |
| market    | always                                                                 | `marketPx + sign(or.dir \|\| -sign(size)) * slip`                          | `strategy.ts:888`     |
| limit     | `o<=p` / `o>=p` (gap)                                                  | `o` (better; no slippage)                                                  | `strategy.ts:892`     |
| limit     | `l<=p` / `h>=p` (touch)                                                | `p` (no slippage)                                                          | `strategy.ts:893`     |
| stop      | `o>=p` / `o<=p` (gap)                                                  | `o + or.dir*slip` (adverse)                                                | `strategy.ts:898`     |
| stop      | `h>=p` / `l<=p` (touch)                                                | `p + or.dir*slip` (adverse)                                                | `strategy.ts:899`     |
| stoplimit | stop arms (`h>=price`/`l<=price`), then limit at `l<=limit`/`h>=limit` | `limit` (or `o` if resting from a **prior** tick and gapped) — no slippage | `strategy.ts:901-917` |

For a market close/closeAll order `or.dir` is 0, so `or.dir || -sign(this.size)` falls back to the direction opposite the held position. Stop-limit's open-gap-to-open branch applies only when the limit was already resting from a prior tick (`wasTriggered`); a stop armed on the current tick can only fill at its limit price within the range. After Phase 1, `this.pending = stillPending;` (`strategy.ts:919`).

**Phase 2 — exit brackets** (`strategy.ts:921-922`): `if (size !== 0 && exits.length) processExits(o, h, l)`.

**Phase 2b — max_intraday_filled_orders cap** (`strategy.ts:924-931`): if `riskDayFills >= riskMaxIntradayOrders` and not halted, `riskTrip(true)` (§13).

**Phase 3 — mark-to-market + margin** (`strategy.ts:933-935`): `markToMarket(o, h, l)` updates the equity curve and drawdown/run-up extremes along the assumed path (§10) and calls `marginCheck` (a margin call can force-close mid-bar, §8); then `this.riskBarCloseEquity = this.equity`.

### `processExits(o, h, l)` (`strategy.ts:1052-1175`)

Setup:

- Intrabar path heuristic: `const highFirst = h - o < o - l;` (`strategy.ts:1056`) — the extreme nearer the open is assumed hit first.
- `firstOf(a, b, wantHigh)` (`strategy.ts:1060-1065`): `a==null?b : b==null?a : wantHigh?Math.max(a,b):Math.min(a,b)` — when an absolute price and a tick-distance resolve the same side, pick the one nearer the market (first to trigger).
...
Per-lot tick levels are measured from **that lot's own fill price** (`lot.price`); absolute `ex.stop`/`ex.limit` are shared. Fill resolution, in exact order (`strategy.ts:1122-1140`):

```ts
if (stopPx != null && (dir === LONG ? o <= stopPx : o >= stopPx)) {
  book(lot, take, o - dir * slip);
  continue;
} // open gap
if (limitPx != null && (dir === LONG ? o >= limitPx : o <= limitPx)) {
  book(lot, take, o);
  continue;
} // open gap
...
### Trailing stop — `trailFill(ex, dir, o, h, l)` (`strategy.ts:1186-1214`)

Position-aggregate. Armed only when `trailOffset != null && (trailPoints != null || trailPrice != null)`. With `off = ex.trailOffset * mt`:

```ts
const act =
  ex.trailPrice != null ? ex.trailPrice : this.avgPrice + dir * (ex.trailPoints ?? 0) * mt; // :1152-1153
...
for (const p of path) {
  ...
  if (NaN(stop)) {
    if (dir === LONG ? p >= act : p <= act) stop = cand;
  } // arm  :1163-1164
  else stop = dir === LONG ? Math.max(stop, cand) : Math.min(stop, cand); // ratchet  :1166
}
```

Walks `open → nearer extreme → farther extreme → close`, arms when the path reaches `act`, ratchets `trail_offset` behind favorable prices (never loosening), reports a hit the moment the path crosses the ratcheted level. Fill applies adverse slippage: `px = fillPx - sign(size) * slip` (`strategy.ts:1152`). A low occurring before arming cannot trigger; the ratchet cannot use an extreme the path has not yet reached.

### Fill dispatch

`fill(o, price)` (`strategy.ts:1218-1225`) calls `execute` then increments `riskDayFills` only when the position/lots/closed-trades actually changed. `execute` (`strategy.ts:1227-1275`) routes closeAll/close/reversal/netting/open per §4, applying the pyramiding cap and `max_position_size` clamp before `openOrAdd` (which applies the margin gate, §8).
...
Single source of truth, `commission(qty, price)` (`strategy.ts:552-558`):
...
| ------------------- | ------------------------------------- | ----------------- | ------------------------------------------------------------------------------------------------------- |
| `percent`           | `(commissionValue/100) * qty * price` | `strategy.ts:555` | Percent of fill notional; `commissionValue` is a whole-number percent (`0.1` → 0.1%). Charged per side. |
| `cash_per_contract` | `commissionValue * qty`               | `strategy.ts:556` | Flat cash per contract; price-independent.                                                              |
| `cash_per_order`    | `commissionValue`                     | `strategy.ts:557` | Flat cash per order; qty- and price-independent.                                                        |
| any                 | `0` if `commissionValue <= 0`         | `strategy.ts:554` | Default value 0 → no-arg strategies pay nothing.                                                        |

`qty` is the unsigned contracts in the fill; `price` is the caller's fill price (already slippage-adjusted for stops/trailing, `strategy.ts:1137`, `1109`), so `percent` commission is charged on the **post-slippage** notional (parity: TV's pre/post-slippage basis is not verified here — §16).

**Entry side** — `openOrAdd()` books the fee once, stores it on the lot, debits realized, accumulates the total (`strategy.ts:1279`, `1262`, `1275-1276`):
...
**Exit side** — `closeLot()` (`strategy.ts:1380-1424`), defaulting the exit fee to a standalone order's commission but accepting an explicit pro-rated share:
...
Double-count avoidance: the entry fee was subtracted from `realized` at open (`strategy.ts:1318`); on close `realized += profit + entryFee` re-adds it, leaving realized reduced by only the exit fee this step. Net across a round trip, realized is reduced by `entryFee + exitFee` exactly once. `netProfit === realized` (`strategy.ts:457-458`) is fully commission-net both sides. `totalCommission` (`strategy.ts:160`, `312`) accumulates both sides = TradingView's "Commission Paid", reported as `StrategyReport.totalCommission` (`strategy.ts:1339`).

**Attribution.** Each `ClosedTrade` row stores `commission: entryFee + exitFee` (`strategy.ts:1411`). Partial close attributes only the pro-rated shares for `take` contracts; the remaining lot fee stays on the still-open lot (`strategy.ts:1387-1388`). Readbacks: `strategy.closedtrades.commission(k)` → `t.commission`; `strategy.opentrades.commission(k)` → `lot.fee` (remaining un-attributed entry commission, `strategy.ts:831-832`).
...
- Market/`strategy.close`: `totalFee = commission(closeQty, price)`; per lot `closeLot(lot, take, price, totalFee * (take/closeQty))` (`strategy.ts:1363`, `1323`).
- Trailing-stop group: `book(lot, take, px, commission(group, px) * (take/group))` (`strategy.ts:1162`).
- Individual bracket fills call `book(lot, take, px)` with no fee arg → `closeLot` default `commission(take, price)` (`strategy.ts:1384`).

For `percent`/`cash_per_contract` the pro-rating is exact (linear in qty). **No rounding anywhere** in the commission path; the only nearby epsilon is the `1e-9` margin-gate comparison tolerance (`strategy.ts:1295`), not a fee rounding.

---

## 7. Slippage & tick rounding

`slippage` is expressed in **ticks** and converted to a price offset once per pass: `slip = settings.slippage * host.mintick` (`strategy.ts:882`, exit path `:1010-1011`). `mintick` lives on the context (`context.ts:369`, default `0.01`, settable per-run via `RunOptions.mintick`), surfaced as `syminfo.mintick`. `StrategyHost` (`strategy.ts:37-48`) exposes `open,high,low,close,time,idx,mintick,tradingDayKey` — **no `pointvalue`**.
...
| -------------------------- | ------------------------------------------------- | -------------------------- |
| market                     | `marketPx + sign(or.dir \|\| -sign(size)) * slip` | `strategy.ts:888`          |
| pending stop (gap / touch) | `o + or.dir*slip` / `p + or.dir*slip`             | `strategy.ts:898-899`      |
| pending limit              | `o` / `p` (no slip)                               | `strategy.ts:892-893`      |
| stop-limit (limit leg)     | `o` / `or.limit` (no slip)                        | `strategy.ts:911-912`      |
| exit stop (gap / touch)    | `o - dir*slip` / `stopPx - dir*slip`              | `strategy.ts:1125`, `1094` |
| exit limit                 | `o` / `limitPx` (no slip)                         | `strategy.ts:1129`, `1097` |
| trailing stop              | `fillPx - sign(size)*slip`                        | `strategy.ts:1152`         |
...
**Tick-distance → price conversions** (via `mintick`, applied _before_ slippage): exit loss/profit `lot.price ∓ dir * ex.loss/profit * mt` (`strategy.ts:1113`, `1075`); trailing `off = ex.trailOffset * mt`, `act = ex.trailPrice ?? avgPrice + dir*(ex.trailPoints ?? 0)*mt` (`strategy.ts:1193-1196`).

**Tick rounding of prices: none.** Executed fill prices flow straight through, never snapped to mintick. The **only** `Math.round(raw/mt)*mt` in the file is on the derived, reported `marginLiquidationPrice` (`strategy.ts:477-478`, guarded `mt > 0` else raw).

**Point value = 1 (implicit).** PnL is the raw price delta × quantity — there is no tick-value/contract-multiplier. Closed-trade profit `dir*(price - lot.price)*take` (`strategy.ts:1389`); open PnL `size*(close - avgPrice)` (`strategy.ts:437`); margin/liquidation math explicitly documented `PointValue = 1` (`strategy.ts:462`, `971`). `syminfo.pointvalue` is hardcoded `1` (`context.ts:637`) and exposed to scripts but never read by the broker. So a `slip` price offset costs exactly `slip × qty` in currency. Instruments with point value ≠ 1 are not modeled (§16).
...
### The `Account` class (`strategy.ts:184-248`)
...
| `broadcastMarks(pts, marker)` | `∀ b ≠ marker: b.foldEquityMarks(pts)` — every pot mark reaches every tracker (spec S7; no-op for a private account) | `strategy.ts:245-247`; observer fold `strategy.ts:996-1011` |
...
The broker starts with a private account seeded from its own initial: `account = new Account(settings.initialCapital).attach(this)` (`strategy.ts:269`). A portfolio host calls `setAccount(a)` (`strategy.ts:276-282`) to swap in a shared pot, sets `ownAccount = false`, and re-seeds `peakEquity`/`valleyEquity` from `a.initial`.

| Getter                  | Formula                                               | Code                  |
| ----------------------- | ----------------------------------------------------- | --------------------- |
| `valuationPrice`        | `Number.isNaN(host.close) ? lastMark : host.close`    | `strategy.ts:443-446` |
| `accountOpenProfit`     | `size === 0 ? 0 : size * (valuationPrice - avgPrice)` | `strategy.ts:448-450` |
| `accountRequiredMargin` | `size === 0 ? 0 : m <= 0 ? 0 :                        | size                  | * valuationPrice * m` | `strategy.ts:452-456` |
| `avgPrice`              | `Σ(qty·price)/Σqty`, `NaN` while flat                 | `strategy.ts:495-503` |

### Margin liquidation price (`strategy.margin_liquidation_price`)

`marginLiquidationPrice` (`strategy.ts:467-479`), Help-Center formula (PointValue = 1) `P = ((initialCapital + netProfit)/|size| − D·avgPrice)/(m − D)`, generalized to the free pot:
...
### Entry-time funds gate (`openOrAdd`, `strategy.ts:1278-1296`)
...
(`strategy.ts:495-503`) A partial FIFO close removes/shrinks lots so the remainder re-prices automatically. Returns `NaN` while flat (`position_avg_price` surfaces NaN via explicit guard, `strategy.ts:1522-1523`).

### Opening / adding (`openOrAdd`, `strategy.ts:1278-1320`)

After the margin gate (§8), pushes a `Lot`; `flat: size = dir*qty; add: size += dir*qty` (`strategy.ts:1309-1316`). Immediately after the size change it calls `recordExposure()` (`strategy.ts:1317`) to advance the peak-position maxima (§12), then debits the entry fee to realized and carries it per-lot (`strategy.ts:1318-1319`). On an add, average price is not stored — the pushed lot changes the weighted mean.

### Closing — FIFO matching

`closePosition(price, qty?, entryId?)` (`strategy.ts:1355-1371`): filters lots to `entryId` if given, computes `eligible = Σ lt.qty` (no-op if `≤ 0`), `closeQty = (qty valid) ? min(qty, eligible) : eligible`, one order-level `totalFee = commission(closeQty, price)`, then consumes lots in list order calling `closeLot(lot, take, price, totalFee * take/closeQty)`.

`closeLot` core money math (`strategy.ts:1380-1424`):

- **Realized PnL:** `profit = dir * (price - lot.price) * take - exitFee - entryFee` (`strategy.ts:1389`), `dir = sign(this.size)` (position direction at close). No point-value multiplier.
- **realized update:** `realized += profit + entryFee` (`strategy.ts:1390`) — nets to the exit side only for this step (entry fee pre-debited at open).
- **cumProfit** (`strategy.ts:1410`) = `this.realized` after the update.
- **classification** (`strategy.ts:1392-1398`): `profit>0` → `grossProfit += profit; wins++`; `profit<0` → `grossLoss += -profit` (positive magnitude); `losses++`; else `evens++`.
- **row:** `commission: entryFee + exitFee`, `maxRunup: lot.maxFavMove * take`, `maxDrawdown: lot.maxAdvMove * take`.
- **size update** (`strategy.ts:1417-1423`): `this.size = dir * (|size| - take)`; lot spliced when `lot.qty <= 1e-9`; snaps to flat and clears `entryId`/`entryLots`/`exits` when `|size| <= 1e-9` or no lots remain.

### Reversal vs netting

- **`strategy.entry`** reversal (`strategy.ts:1250-1251`, `1225-1231`): `closePosition(price)` flattens fully, then falls through to open the **full** fresh qty.
- **`strategy.order`** netting (`strategy.ts:1254-1260`): `closable = min(qty, |size|); closePosition(price, closable); rem = qty - closable; if (rem>0) openOrAdd(o, dir, rem, price)`.
- `risk.allow_entry_in` against the allowed direction (`strategy.ts:1242-1248`): closes an opposite position (no reversal), no-op when flat.

### Per-trade run-up (MFE) & drawdown (MAE)

Accrued per-lot, per-contract, in `markToMarket` (`strategy.ts:974-982`), only while `size !== 0` and only for lots still open (a lot removed by this pass's fills already ended its life at its exit fill, `strategy.ts:971-973`):
...
On close the ledger multiplies by the closed quantity (`maxRunup: lot.maxFavMove * take`, `strategy.ts:1412-1413`).
...
Written once per bar inside `markToMarket(o, h, l)` (`strategy.ts:941-986`), called from `processTick` Phase 3.

**Per-bar equity mark** (`strategy.ts:943-945`):
...
`equityCurve` is a sparse array indexed by global bar index (`strategy.ts:163-164`), unset before the strategy activated. A mid-bar margin call re-marks the point with the post-call position (`strategy.ts:1049`); the `process_orders_on_close` pass re-runs `processTick` and overwrites the same bar's value.

**Drawdown & run-up extremes** (intrabar, `strategy.ts:946-965`):
...
- `peakEquity`/`valleyEquity` are running high-/low-water marks seeded to `account.initial` by `configure()`/`setAccount()` (`strategy.ts:280-281`, `414-415`; init `strategy.ts:310-311`).
- `maxDrawdown` = max of `peakEquity − v`; `maxRunup` = max of `v − valleyEquity`; percent forms guarded `peakEquity>0` / `valleyEquity>0`.

After the extremes loop, `markToMarket` broadcasts the same `pts` to every OTHER broker attached to the account — `account.broadcastMarks(pts, this)` (`strategy.ts:970`) → each observer's `foldEquityMarks(pts)` (`strategy.ts:996-1011`) runs the identical per-point extreme/day-max math (spec S7: a sleeve on a disjoint clock still observes the pot's peaks; trip _tests_ stay at each sleeve's own marks; no-op for a private account). It then calls `recordExposure()` (`strategy.ts:983`) — one peak-position update per mark — then `riskCheckEquity(pts)` and `marginCheck` (§8, §13). Per-bar peak/valley are updated within the extreme loop in path order, so `dd`/`ru` at a point use the peak/valley seen up to and including that point. These _equity-curve_ extremes are distinct from the per-trade `maxRunup`/`maxDrawdown` on `ClosedTrade` (§9).

`onBar()` counters (`strategy.ts:863-864`): `barsProcessed++`; `if (size !== 0) barsInMarket++` (a position opened only by the `process_orders_on_close` pass counts from the next bar, `strategy.ts:321-323`).

---

## 11. The broker StrategyReport

`report()` (`strategy.ts:1323-1346`) is a **pure snapshot** — every field is a copy of running state; nothing is computed at report time. Derived analytics (Sharpe, Sortino, CAGR, buy-&-hold) live in `computeStrategyMetrics` (`strategy.ts:144-146`).

| Field                       | Source                                                                                                                                                         | Code                           |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------ |
| `initialCapital`            | `account.initial` — identical to `settings.initialCapital` for a private account; the POT under a shared account (matches `strategy.initial_capital`, spec S2) | `strategy.ts:1328`             |
| `netProfit`                 | `this.netProfit` (= `realized`)                                                                                                                                | `strategy.ts:457-458`          |
| `grossProfit`               | `this.grossProfit`                                                                                                                                             | incremental `strategy.ts:1393` |
| `grossLoss`                 | `this.grossLoss` (**positive magnitude**)                                                                                                                      | incremental `strategy.ts:1396` |
| `wins` / `losses` / `evens` | classification counters                                                                                                                                        | `strategy.ts:1392-1398`        |
| `maxDrawdown`               | intrabar-path peak-to-trough cash                                                                                                                              | `strategy.ts:957-958`          |
| `maxDrawdownPercent`        | `(dd/peakEquity)*100` max                                                                                                                                      | `strategy.ts:959-960`          |
| `maxRunup`                  | intrabar-path trough-to-peak cash                                                                                                                              | `strategy.ts:961-962`          |
| `maxRunupPercent`           | `(ru/valleyEquity)*100` max                                                                                                                                    | `strategy.ts:963-964`          |
| `totalCommission`           | both sides = TV "Commission Paid"                                                                                                                              | `strategy.ts:1319`, `1346`     |
| `closedTrades`              | `ClosedTrade[]`, one row per closed lot (FIFO)                                                                                                                 | `strategy.ts:1350`             |
| `equityCurve`               | sparse, indexed by global bar idx                                                                                                                              | `strategy.ts:943-945`          |
| `barsProcessed`             | active-bar count (exposure denominator)                                                                                                                        | `strategy.ts:863`              |
| `barsInMarket`              | bars with an open position after the open fill pass                                                                                                            | `strategy.ts:864`              |
| `marginCalls`               | `marginCallCount` (0 with margins off)                                                                                                                         | `strategy.ts:1344`             |
...
### `tradeStat(scope, field)` (`strategy.ts:535-542`)
...
### `tradeField(scope, field, i)` (`strategy.ts:766-844)`, `k = Math.trunc(i)`
...
Both return `NaN` for an out-of-range index or an untracked field (`entry_comment`/`exit_comment`/`exit_id` are not tracked, `strategy.ts:806`).

`tradePct(amount, entryPrice, qty)` (`strategy.ts:760-763`): `basis = |entryPrice*qty|; return basis>0 ? (amount/basis)*100 : 0`.

### The strategy.* accessor namespace (`makeStrategyNs`, `strategy.ts:1431-1625`)

| Pine variable                       | Getter body                               | Line      |
| ----------------------------------- | ----------------------------------------- | --------- |
| `strategy.position_size`            | `b.size` (signed)                         | 1519-1521 |
| `strategy.position_avg_price`       | `b.size === 0 ? NaN : b.avgPrice`         | 1522-1524 |
| `strategy.equity`                   | `b.equity` (= `account.equity`)           | 1525-1527 |
| `strategy.openprofit`               | `b.openProfit`                            | 1528-1530 |
| `strategy.netprofit`                | `b.netProfit` (= `realized`)              | 1531-1533 |
| `strategy.grossprofit`              | `b.grossProfit`                           | 1534-1536 |
| `strategy.grossloss`                | `b.grossLoss` (positive magnitude)        | 1537-1539 |
| `strategy.wintrades`                | `b.wins`                                  | 1540-1542 |
| `strategy.losstrades`               | `b.losses`                                | 1543-1545 |
| `strategy.eventrades`               | `b.evens`                                 | 1546-1548 |
| `strategy.closedtrades`             | `b.closedTrades.length`                   | 1549-1551 |
| `strategy.opentrades`               | `b.openTradeCount` (= `entryLots.length`) | 1552-1554 |
| `strategy.initial_capital`          | `b.account.initial`                       | 1557-1559 |
| `strategy.max_drawdown`             | `b.maxDrawdown`                           | 1560-1562 |
| `strategy.max_drawdown_percent`     | `b.maxDrawdownPercent`                    | 1580-1582 |
| `strategy.max_runup`                | `b.maxRunup`                              | 1583-1585 |
| `strategy.max_runup_percent`        | `b.maxRunupPercent`                       | 1586-1588 |
| `strategy.max_contracts_held_all`   | `b.maxContractsAll`                       | 1607-1609 |
| `strategy.max_contracts_held_long`  | `b.maxContractsLong`                      | 1610-1612 |
| `strategy.max_contracts_held_short` | `b.maxContractsShort`                     | 1613-1615 |
| `strategy.position_entry_name`      | `b.positionEntryName`                     | 1616-1618 |
| `strategy.margin_liquidation_price` | `b.marginLiquidationPrice`                | 1621-1623 |
| `strategy.account_currency`         | `'USD'` (constant)                        | 1563-1565 |

**Peak-position exposure** (`recordExposure`, `strategy.ts:544-550`) tracks three running maxima seeded to 0 (`strategy.ts:318-320`) and in `SNAP_SCALARS` (`strategy.ts:384-386`, so they roll back cleanly on a superseded realtime tick):
...
It is invoked after **every** size change — once per fill in `openOrAdd` (`strategy.ts:1317`) and once per mark in `markToMarket` (`strategy.ts:983`) — so intrabar peaks reached by a margin-call-reduced position are still captured. These are **report-only** scalars (TV's "Max contracts held"): they are surfaced only through the three `strategy.max_contracts_held_*` accessors above and are **not** part of `StrategyReport` (§11) nor `computeStrategyMetrics` (§14).

**Percent-of-initial-capital getters** (`strategy.ts:1568-1579`), all guarded on `b.account.initial` truthiness else 0: `netprofit_percent`, `openprofit_percent`, `grossprofit_percent`, `grossloss_percent` = `(metric / account.initial) * 100`.

**Cash averages** (`strategy.ts:1589-1606`): `avg_trade = closedTrades.length ? netProfit/closedTrades.length : 0`; `avg_winning_trade = wins ? grossProfit/wins : 0`; `avg_losing_trade = losses ? grossLoss/losses : 0`.

**Percent averages** delegate to `pct()`/`meanPct()` (`strategy.ts:514-531`):
...
**Convenience metrics** (not Pine builtins): `profitFactor` (`strategy.ts:480-486`): `gl = |grossLoss|; gl>0 ? grossProfit/gl : grossProfit>0 ? Infinity : 0`. `winRate` (`strategy.ts:487-492`): `decided = wins+losses; decided>0 ? wins/decided : 0`.

---

## 13. Risk management rules

Six `strategy.risk.*` rules, configured by setters (`strategy.ts:662-689`, each early-returns if `!this.active`) and enforced at two kinds of point: (a) entry/quantity gates inside `execute()` at fill time, and (b) "halt" rules that call `riskTrip()`.

**Most-restrictive merge** — all numeric rules funnel through `riskMin` (`strategy.ts:658-661`): `v==null||NaN(v) ? cur : (cur==null ? v : Math.min(cur, v))`. `allow_entry_in` is the exception: last-valid-value assignment (only `'all'|'long'|'short'` accepted). Namespace wiring `strategy.ts:1451-1461`.

**Halt machinery.** `riskHaltActive` (`strategy.ts:692-694`): `this.riskHalted || this.riskHaltedDay === this.riskDay`. `riskTrip(untilDayEnd)` (`strategy.ts:702-707`): sets the halt flag (permanent or day-scoped), **cancels every pending order and exit bracket**, and submits **one emergency `closeAll` market order** (only if a position is open, fills next tick pass). While halted, every public verb (`entry`/`order`/`close`/`close_all`/`exit`) early-returns at submit time.

**Trading-day rollover** (`riskRollDay`, `strategy.ts:712-728`), called at the start of every `onBar()`: on a new `tradingDayKey`, score the day that just closed (loss day = `riskBarCloseEquity < riskDayStartEquity - 1e-9`; any non-loss day resets the streak), then reset day baselines and `riskDayFills`. The new day's baselines are seeded from the last bar-close equity **when finite, else from the account initial** — this matters for the very first trading day, when `riskBarCloseEquity` is still NaN before any bar has closed: `eq = Number.isFinite(riskBarCloseEquity) ? riskBarCloseEquity : account.initial; riskDayStartEquity = riskDayMaxEquity = eq; riskDayFills = 0` (`strategy.ts:721-727`). `tradingDayKey` (`context.ts:456-460`): bar index `idx` for timeframes > 1 day, else `tradingDayMs(time)`.

**Equity-based rules** run in `riskCheckEquity(pts)` (`strategy.ts:734-757`) at the end of `markToMarket`, over the bar's intrabar equity path points.

| Rule                         | Baseline / trigger metric     | Comparison                                  | Action                                          | Code                    |
| ---------------------------- | ----------------------------- | ------------------------------------------- | ----------------------------------------------- | ----------------------- |
| `allow_entry_in`             | order direction vs allowed    | `dir !== allowed`                           | block entry; close opposite if held, else no-op | `strategy.ts:1242-1248` |
| `max_position_size`          | resulting `                   | size                                        | `                                               | `qty = min(qty, cap -   | size | )`; drop if ≤0 | clamp entry qty | `strategy.ts:1271-1272` |
| `max_cons_loss_days`         | consecutive loss days         | `riskConsLossDays >= cap`                   | `riskTrip(false)` — **permanent**               | `strategy.ts:718-719`   |
| `max_drawdown` (cash)        | strategy peak-to-trough       | `maxDrawdown >= cap - 1e-9`                 | `riskTrip(false)` — **permanent**               | `strategy.ts:736-737`   |
| `max_drawdown` (%)           | `dd / peakEquity * 100`       | `maxDrawdownPercent >= cap - 1e-9`          | `riskTrip(false)` — **permanent**               | `strategy.ts:738`       |
| `max_intraday_loss` (cash)   | `dayStartEquity - v`          | `loss >= cap - 1e-9`                        | `riskTrip(true)` — **day halt**                 | `strategy.ts:747-749`   |
| `max_intraday_loss` (%)      | `loss` vs % of `dayMaxEquity` | `loss >= (cap/100)*riskDayMaxEquity - 1e-9` | `riskTrip(true)` — **day halt**                 | `strategy.ts:750-751`   |
| `max_intraday_filled_orders` | `riskDayFills`                | `riskDayFills >= cap` (and not halted)      | `riskTrip(true)` — **day halt**                 | `strategy.ts:926-931`   |

Notes: `max_drawdown` uses the **strategy-wide** peak-to-trough (§10), not the day's; percent is a share of running peak (max) equity. `riskCheckEquity` walks the intrabar `pts` in path order, and at each point it **first** raises `riskDayMaxEquity` (`if (v > riskDayMaxEquity) riskDayMaxEquity = v`, `strategy.ts:745`) and **then** — unless the day is already halted (`dayHalted = riskHaltedDay === riskDay`, `strategy.ts:743`; `if (dayHalted) continue`, `strategy.ts:746`) — tests the same point's `loss = riskDayStartEquity - v` against the cash cap and against `(pct/100)*riskDayMaxEquity` (`strategy.ts:747-751`). So `max_intraday_loss`'s percent basis is the day's **maximum** equity (`riskDayMaxEquity`), while the loss reference is the day's **opening** equity (`riskDayStartEquity`) — an intentional v6-reference detail (`strategy.ts:730-733`); open question whether this exactly matches TV. Crucially, the max-equity ratchet **keeps advancing even during a day halt** (only the loss trip-tests are gated by `dayHalted`), so a later un-halted bar in the same day compares against the correct running max. `allow_entry_in` closes an opposite position without reversing. `riskDayFills` (`fill()`, `strategy.ts:1218-1225`) counts only orders that actually trade (position/lots/closed-trades changed) and each lot's exit on a multi-lot close. Forced **margin liquidation** (§8) is a separate mechanism, explicitly **NOT** a halt. **Shared account:** the peak/valley/day-max trackers these rules read are fed by _every_ pot mark, not just this broker's own bars — `Account.broadcastMarks` folds each marking broker's `pts` into all other attached brokers (`foldEquityMarks`; same per-point math, no trip tests), so a sleeve on a disjoint clock cannot under-measure the shared drawdown (spec S7; V4 disjoint-clock fixture).
...
- **isolated** (default, `:98`): each sleeve keeps its private `Account`, funded `wᵢ·P` via the `EngineOptions.strategy` override (`initialCapital: weights[i]*capital`, `:122`). Reproduces equal-/weighted models; gate V3 asserts the portfolio curve equals the forward-fill-and-sum oracle bit-for-bit.
- **shared**: one `Account(P)` swapped into every broker (`new Account(capital)` `:125`; `for (e) e.ctx.strategyBroker.setAccount(shared)` `:126`) so sizing, funds checks, margin, and risk read portfolio equity.

Pot: `capital = opts.capital ?? n * (script.metadata.strategy?.initialCapital ?? 1_000_000)` (`:106-107`; rejected if not finite or ≤ 0). `normalizedWeights(weights, n)` (`:253-263`): `undefined` → equal weight `1/n`; length mismatch or non-finite/≤0 weight throws; else `w/total`. Weights are **isolated-only**.

**Per-sleeve `request.security` injection** (`portfolio-engine.ts:127-132`): before each engine's `prepare()`, the run copies that sleeve's host-fetched security bars into the engine's context — `if (s.securityBars) for (const [key, bars] of Object.entries(s.securityBars)) engines[i].ctx.securityBars.set(key, bars)`. Keys are `SYMBOL@TF` (`PortfolioSleeveSpec.securityBars`, a `Record<string, Bar[]>`, `portfolio-engine.ts:44-46`), injected exactly as a single-symbol host does — so `request.security(...)` resolves correctly inside a portfolio run.

Master clock: `unionTimes` (`:267-271`) = `Array.from(new Set(all sleeve bar times)).sort((a,b)=>a-b)`. The loop (`:147-178`) walks the master axis with a per-sleeve `cursor`; at each master time `t`, every sleeve whose current bar's `time === t` is `step()`ed **in basket order** (spec S4 — earlier sleeves' fills settle before later sleeves size), advancing its cursor and recording `lastStepped`. Sleeves execute only on their own bars (spec S8).

### Portfolio equity curve (`:161-171`)

Read from **recorded** equity marks, never live getters (between bars the series pointer sits past the committed bar, so live `host.close`/`openProfit` is NaN, `:138-146`):

- **shared** (`:162-163`): `equityCurve[k] = engines[lastStepped].ctx.strategyBroker.equityCurve[cursor[lastStepped]-1]` — every broker's mark under the shared account IS portfolio equity; the last-stepped mark is most complete.
- **isolated** (`:166-170`): `eq += cursor[i] > 0 ? b.equityCurve[cursor[i]-1] : weights[i]*capital` — Σ of each sleeve's last recorded mark, using funding `weights[i]*capital` before its first bar (pre-activation cash), last mark after data ends (ragged tail). This is the forward-fill-and-sum oracle evaluated at run time.

`barsInMarket` (`:172-177`) increments once per master bar if ANY sleeve's broker `size !== 0` after the fill pass.

**Risk rules across sleeves (spec S7).** Every `markToMarket` broadcasts its
equity path to all other brokers on the shared account (§10, §13), so each
sleeve's `strategy.risk.*` trackers observe the full pot path — including marks
at master times where that sleeve has no bar — and trip at the sleeve's own
next mark. The portfolio therefore halts within one bar per sleeve even on
disjoint clocks (pinned by the V4 disjoint-clock fixture).

### Portfolio StrategyReport (`:181-228`)

Plain sums via `const sum = (f) => reports.reduce((a,r)=>a+f(r),0)` (`:188`) for `netProfit`, `grossProfit`, `grossLoss`, `wins`, `losses`, `evens`, `totalCommission`, `marginCalls`. Merged ledger (`:192-197`): each sleeve's `closedTrades` copied `{...t, symbol: s.symbol}`, sorted `(a,b) => a.exitTime - b.exitTime` (stable → basket order on ties), then `cumProfit` re-accumulated portfolio-wide (`for (t) t.cumProfit = cum += t.profit`). `barsProcessed: times.length`.

**Combined drawdown/run-up** — `closeToCloseExtremes(equityCurve, capital)` (`:276-295`), peak/valley seeded at the pot, NaN skipped:
...
**Derived metrics** (`metrics`, `:238-248`): `computeStrategyMetrics(r.report, { barTimes: r.times, timeframeSeconds: tfSeconds(this.lastTf), ...opts })`. All sleeves share one timeframe by contract; **no `bars` benchmark** (buy-&-hold is meaningless for a basket).

### 15.2 Realtime / rollback interaction & the snapshot taxonomy

`markToMarket` records bar-close equity (`equityCurve[host.idx] = eq`, `:942`) which the portfolio aggregation reads by cursor; the driver rolls back `equityCurve` via a `SNAP_APPEND` slice, so an uncommitted realtime tick's mark is discarded on the next rollback — portfolio aggregation never sees speculative equity.
...
Broker `snapshot()`/`restore()` (`strategy.ts:409-422`) use a three-way field taxonomy (declared `:363-405`) so rollback stays correct without deep-cloning the ever-growing history arrays each tick:
...
- **No contract flooring.** `cash` and `percent_of_equity` sizing return fractional contracts (`defaultQty`, `strategy.ts:566-571`). TV floors to whole contracts. Known deviation.
...
- Intrabar path heuristic (extreme nearer the open assumed first) mirrors TV's bar-magnifier-off assumption; used for exit stop/limit ordering, equity drawdown/run-up, and the margin-call walk (`strategy.ts:1056`, `937`, `988`).
...
**Flagged parity questions (open, from source comments):** _(P3 was retired during review; the P-series is intentionally non-contiguous — P1, P2, P4.)_

- **P1** — margin-call fill price uses the violating intrabar path point `p` (`strategy.ts:1025`).
- **P2** — `marginLiquidationPrice` tick-rounding (`strategy.ts:466`).
- **P4** — the margin affordability gate counts the entry fee against available equity (`strategy.ts:1285-1286`).
- **S4** — under a shared account the opening margin gate treats the rest of the basket's margin as first-come-first-served (`strategy.ts:1291-1294`), possibly differing from TV per-symbol accounting.
...
| Position sizing (`defaultQty`, `qtyFor`)                                   | `strategy.ts:560-571`                                                                                   |
| Order record & `orderTrigger`                                              | `strategy.ts:27-32`, `71-83`                                                                            |
| `entry`/`order`/`close`/`close_all`/`exit`/`submit`                        | `strategy.ts:575-654`                                                                                   |
| `cancel`/`cancel_all`                                                      | `strategy.ts:847-854`                                                                                   |
| Fill passes (`onBar`/`onBarClose`/`processTick`)                           | `strategy.ts:858-936`                                                                                   |
| Exit brackets (`processExits`) / trailing (`trailFill`)                    | `strategy.ts:1052-1175`, `1143-1171`                                                                    |
| Fill dispatch (`fill`/`execute`/`openOrAdd`)                               | `strategy.ts:1218-1320`                                                                                 |
| Closing (`closePosition`/`closeLot`)                                       | `strategy.ts:1355-1424`                                                                                 |
| Commission                                                                 | `strategy.ts:552-558`                                                                                   |
| `Account` class                                                            | `strategy.ts:184-248`                                                                                   |
| Margin (gate/liquidation/marginCheck)                                      | `strategy.ts:451-479`, `983-1007`, `1244-1252`                                                          |
| Avg price / open PnL / net profit / equity getters                         | `strategy.ts:433-503`                                                                                   |
| Mark-to-market, equity curve, extremes                                     | `strategy.ts:941-986`                                                                                   |
| Peak-position exposure (`recordExposure`)                                  | `strategy.ts:318-320`, `530-536`, `964`, `1274`                                                         |
| `report()` / `StrategyReport`                                              | `strategy.ts:147-172`, `1280-1301`                                                                      |
| Trade introspection (`tradeStat`/`tradeField`/`tradePct`/`pct`/`meanPct`)  | `strategy.ts:514-542`, `746-830`                                                                        |
| `strategy.*` namespace (`makeStrategyNs`)                                  | `strategy.ts:1431-1625`                                                                                 |
| Risk rules (setters/riskMin/riskTrip/riskRollDay/riskCheckEquity)          | `strategy.ts:658-757`, `910-917`, `1196-1230`                                                           |
| `profitFactor`/`winRate`                                                   | `strategy.ts:480-492`                                                                                   |
| snapshot/restore taxonomy                                                  | `strategy.ts:366-422`                                                                                   |
