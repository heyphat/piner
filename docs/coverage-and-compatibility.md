...
- **Phase 8 — `strategy.*`** ✅ (v1): broker/order engine — next-bar-open fills,
  ...
  Tail: trailing stops, per-trade introspection, OCA (`calc_on_order_fills`
  landed later — historical four-tick intrabar re-execution).
...
- **Tails (genuine gaps):**
  history on inline expressions, strategy OCA groups, live
  `syminfo` metadata, cross-symbol/realtime `request.security` + real `request.*`
  data feeds. (`calc_on_every_tick` is a deliberate no-op, not a gap — it only
  changes realtime behavior on TV, and the stack is historical-only.)
