...
> **TL;DR.** The full Pine v6 *language* (lexer/parser/semantics, expressions,
> control flow, types, `var/varip`, history, `na`, UDFs, UDTs, enums, collections)
> is implemented, plus the complete *visual* surface (plots, markers, candles,
...
> **97.8% of single-script examples compile *and run* end-to-end** (399/408), the codegen and
...
1. **Two-backend oracle.** Every test compiles to JS *and* runs the AST interpreter
   ...
   lowering/codegen divergence — but not a *wrong-but-consistent* runtime impl (both
...
| `na` semantics | ✅ | propagation in arithmetic; comparisons → `false` (v6); `==na` lint. **`na(x)` is the is-na *function*** (parser fix — was mis-tokenized as the truthy `na` literal) |
...
gap as *fillable now* vs *deferred* (needs a data feed/larger subsystem). Current
...
currency codes all wired). The remaining 38 gaps are exclusively *deferred*: Variables
...
real *function* gaps were small mechanical ones — now closed: `ta.max`/`ta.min`,
...
so a definitive diff against the *current* manual needs an updated export of it. (`piner`
...
**Gap-report sweep — every *fillable* gap closed (95.7% total).** A pass over the
...
(`test/coverage-gaps.test.ts`). The 38 remaining gaps are all *deferred* — they
