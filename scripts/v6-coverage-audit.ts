...
const taM = new Set(
  Object.getOwnPropertyNames(Ta.prototype).filter((n) => n !== 'constructor' && !n.startsWith('_')),
);
...
const fnNs: Record<string, Set<string>> = {
  ta: taM,
  math: keys(ctx.math),
  str: keys(ctx.str),
  color: keys(ctx.color),
  array: keys(ctx.array),
  map: keys(ctx.map),
  matrix: keys(ctx.matrix),
  input: keys(ctx.input),
  line: keys(ctx.line),
  label: keys(ctx.label),
  box: keys(ctx.box),
  table: keys(ctx.table),
  linefill: keys(ctx.linefill),
  polyline: keys(ctx.polyline),
  strategy: new Set([...keys(ctx.strategy), 'tradeField']),
  request: new Set([...keys(ctx.request), 'security']),
  ticker: keys(ctx.ticker),
  log: keys(ctx.log),
  timeframe: keys(ctx.timeframe),
  chart: new Set(['point']),
  syminfo: new Set(['prefix', 'ticker']),
  runtime: keys(ctx.runtime),
};
const GLOBAL_FN_OK = new Set([
  'indicator',
  'strategy',
  'library',
  'plot',
...
const mark = (sec: string, name: string, ok: boolean) => {
  (ok ? (covered[sec] ??= []) : (gaps[sec] ??= [])).push(name);
};

for (const e of entries.Functions ?? []) {
  const cleaned = e.replace(/<[^>]*>/g, '');
  const m = /^([a-z_][\w.]*)\(\)?/.exec(cleaned);
  if (!m) {
    mark('Functions', e, false);
    continue;
  }
  const full = m[1];
  const parts = full.split('.');
  if (parts.length === 1) mark('Functions', full, GLOBAL_FN_OK.has(parts[0]) || !!fnNs[parts[0]]);
    ...
    const extra =
      (ns === 'math' && fn === 'sum') ||
      (ns === 'strategy' && (fn === 'convert_to_account' || fn === 'convert_to_symbol'));
...
for (const e of entries.Keywords ?? []) {
  const k = e.replace('...', ' ');
  mark(
    'Keywords',
    e,
    e.split(/\.\.\.|\s/).every((w) => KEYWORDS.has(w)),
  );
}
const OPS = new Set([...MULTI_OPS, ...SINGLE_OPS, ...PUNCT, '?:', '[]']);
for (const e of entries.Operators ?? [])
  mark('Operators', e, OPS.has(e) || e === '?:' || e === '[]');
...
const DEFER: [RegExp, string][] = [
  ...
  [
    ...
    'live exchange/fundamental metadata (no data feed)',
  ],
...
for (const sec of SECTIONS) {
  const c = covered[sec] ?? [],
    g = gaps[sec] ?? [];
  const tot = c.length + g.length;
  totDoc += tot;
  totCov += c.length;
  const fillable = g.filter((x) => disp(x).fill),
    deferred = g.filter((x) => !disp(x).fill);
  ...
  if (deferred.length) {
    ...
    for (const x of deferred) {
      const w = disp(x).why;
      (byWhy.get(w) ?? byWhy.set(w, []).get(w))!.push(x);
    }
    for (const [w, xs] of byWhy)
...
console.log(
  `TOTAL: ${totCov}/${totDoc} documented entries covered (${((totCov / totDoc) * 100).toFixed(1)}%)`,
);
...
const header =
  ...
  '/' +
  totDoc +
  ' documented entries covered (' +
  ((totCov / totDoc) * 100).toFixed(1) +
  '%).**\n';
writeFileSync(
  new URL('../docs/v6-coverage-gap.md', import.meta.url),
  header + lines.join('\n') + '\n',
);
