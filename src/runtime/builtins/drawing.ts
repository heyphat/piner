  ...
  create(type: DrawType, props: Record<string, unknown>): number {
    ...
    if (cap !== undefined && cap > 0) {
      ...
      for (const o of this.objects.values()) {
        if (o.type === type) { count++; if (oldest < 0) oldest = o.id; }
      }
...
export function makeLineNs(pool: DrawingPool) {
  return {
    get all() { return allOf(pool, 'line'); },
    ...
    new(x1: unknown, y1: unknown, x2?: unknown, y2?: unknown, opts: Opts = {}): number {
      ...
      if (isPoint(x1) && isPoint(y1)) {
        ...
        return pool.create('line', drop({ x1: pxOf(x1, byTime), y1: x1.price, x2: pxOf(y1, byTime), y2: y1.price, ...o }));
      }
      return pool.create('line', drop({ x1, y1, x2, y2, ...opts }));
    },
    copy(id: number): number { return pool.copy(id); },
    set_xy1(id: number, x: number, y: number): void { pool.set(id, 'x1', x); pool.set(id, 'y1', y); },
    set_xy2(id: number, x: number, y: number): void { pool.set(id, 'x2', x); pool.set(id, 'y2', y); },
    // *_point setters take a chart.point; x comes from index/time per the line's xloc, y from price.
    set_first_point(id: number, point: ChartPoint): void { pool.set(id, 'x1', pointX(pool, id, point)); pool.set(id, 'y1', point?.price); },
    set_second_point(id: number, point: ChartPoint): void { pool.set(id, 'x2', pointX(pool, id, point)); pool.set(id, 'y2', point?.price); },
    set_x1(id: number, x: number): void { pool.set(id, 'x1', x); },
    set_y1(id: number, y: number): void { pool.set(id, 'y1', y); },
    set_x2(id: number, x: number): void { pool.set(id, 'x2', x); },
    set_y2(id: number, y: number): void { pool.set(id, 'y2', y); },
    set_color(id: number, c: string): void { pool.set(id, 'color', c); },
    set_width(id: number, w: number): void { pool.set(id, 'width', w); },
    set_style(id: number, s: string): void { pool.set(id, 'style', s); },
    set_extend(id: number, e: string): void { pool.set(id, 'extend', e); },
    // set_xloc updates both x coords and the xloc property in one call.
    set_xloc(id: number, x1: number, x2: number, xloc: string): void {
      pool.set(id, 'x1', x1); pool.set(id, 'x2', x2); pool.set(id, 'xloc', xloc);
    },
    get_x1(id: number): unknown { return pool.get(id, 'x1'); },
    get_y1(id: number): unknown { return pool.get(id, 'y1'); },
    get_x2(id: number): unknown { return pool.get(id, 'x2'); },
    get_y2(id: number): unknown { return pool.get(id, 'y2'); },
    ...
    get_price(id: number, x: number): number {
      const o = pool.objects.get(id);
      if (!o) return NaN;
      const x1 = o.props.x1 as number, y1 = o.props.y1 as number;
      const x2 = o.props.x2 as number, y2 = o.props.y2 as number;
      if (typeof x1 !== 'number' || typeof y1 !== 'number' ||
          typeof x2 !== 'number' || typeof y2 !== 'number') return NaN;
      if (x2 === x1) return y1; // vertical line: degenerate slope
      return y1 + (y2 - y1) * (x - x1) / (x2 - x1);
    },
    delete(id: number): void { pool.remove(id); },
  };
}
...
export function makeLabelNs(pool: DrawingPool) {
  return {
    get all() { return allOf(pool, 'label'); },
    ...
    new(x: unknown, y: unknown, text: unknown = '', opts: Opts = {}): number {
      ...
      if (isPoint(x)) {
        const xlocVal = typeof text === 'string' && text !== '' ? text
          : typeof opts.xloc === 'string' ? opts.xloc : 'bar_index';
        const byTime = xlocVal === 'bar_time';
        const labelText = typeof y === 'string' ? y : y == null ? '' : String(y);
        return pool.create('label', drop({ x: pxOf(x, byTime), y: x.price, text: labelText, xloc: xlocVal, ...opts }));
      }
      return pool.create('label', drop({ x, y, text, ...opts }));
    },
    copy(id: number): number { return pool.copy(id); },
    set_xy(id: number, x: number, y: number): void { pool.set(id, 'x', x); pool.set(id, 'y', y); },
    set_x(id: number, x: number): void { pool.set(id, 'x', x); },
    set_y(id: number, y: number): void { pool.set(id, 'y', y); },
    // set_point takes a chart.point; x comes from index/time per the label's xloc, y from price.
    set_point(id: number, point: ChartPoint): void { pool.set(id, 'x', pointX(pool, id, point)); pool.set(id, 'y', point?.price); },
    set_text(id: number, t: string): void { pool.set(id, 'text', t); },
    set_textalign(id: number, align: string): void { pool.set(id, 'textalign', align); },
    set_tooltip(id: number, tooltip: string): void { pool.set(id, 'tooltip', tooltip); },
    set_text_formatting(id: number, fmt: unknown): void { pool.set(id, 'text_formatting', fmt); },
    set_color(id: number, c: string): void { pool.set(id, 'color', c); },
    set_textcolor(id: number, c: string): void { pool.set(id, 'textcolor', c); },
    set_style(id: number, s: string): void { pool.set(id, 'style', s); },
    set_size(id: number, s: string): void { pool.set(id, 'size', s); },
    set_text_font_family(id: number, family: string): void { pool.set(id, 'text_font_family', family); },
    set_yloc(id: number, yloc: string): void { pool.set(id, 'yloc', yloc); },
    // set_xloc updates the x coord and the xloc property in one call.
    set_xloc(id: number, x: number, xloc: string): void { pool.set(id, 'x', x); pool.set(id, 'xloc', xloc); },
    get_x(id: number): unknown { return pool.get(id, 'x'); },
    get_y(id: number): unknown { return pool.get(id, 'y'); },
    get_text(id: number): unknown { return pool.get(id, 'text'); },
    delete(id: number): void { pool.remove(id); },
  };
}
  ...
  return {
    ...
    new(left: unknown, top: unknown, right?: unknown, bottom?: unknown, opts: Opts = {}): number {
      ...
      if (isPoint(left) && isPoint(top)) {
        ...
        const byTime = o.xloc === 'bar_time';
        return pool.create('box', drop({ left: pxOf(left, byTime), top: left.price, right: pxOf(top, byTime), bottom: top.price, ...o }));
      }
      return pool.create('box', drop({ left, top, right, bottom, ...opts }));
    },
    copy(id: number): number { return pool.copy(id); },
    set_lefttop(id: number, left: number, top: number): void { pool.set(id, 'left', left); pool.set(id, 'top', top); },
    set_rightbottom(id: number, right: number, bottom: number): void { pool.set(id, 'right', right); pool.set(id, 'bottom', bottom); },
    // Corner setters take a chart.point; x (left/right) comes from index/time per the box's xloc, y (top/bottom) from price.
    set_top_left_point(id: number, point: ChartPoint): void { pool.set(id, 'left', pointX(pool, id, point)); pool.set(id, 'top', point?.price); },
    set_bottom_right_point(id: number, point: ChartPoint): void { pool.set(id, 'right', pointX(pool, id, point)); pool.set(id, 'bottom', point?.price); },
    set_left(id: number, left: number): void { pool.set(id, 'left', left); },
    set_top(id: number, top: number): void { pool.set(id, 'top', top); },
    set_right(id: number, right: number): void { pool.set(id, 'right', right); },
    set_bottom(id: number, bottom: number): void { pool.set(id, 'bottom', bottom); },
    set_bgcolor(id: number, c: string): void { pool.set(id, 'bgcolor', c); },
    set_border_color(id: number, c: string): void { pool.set(id, 'border_color', c); },
    set_border_width(id: number, w: number): void { pool.set(id, 'border_width', w); },
    set_border_style(id: number, s: string): void { pool.set(id, 'border_style', s); },
    set_extend(id: number, e: string): void { pool.set(id, 'extend', e); },
    set_text(id: number, t: string): void { pool.set(id, 'text', t); },
    set_text_color(id: number, c: string): void { pool.set(id, 'text_color', c); },
    set_text_font_family(id: number, family: string): void { pool.set(id, 'text_font_family', family); },
    set_text_size(id: number, s: string): void { pool.set(id, 'text_size', s); },
    set_text_halign(id: number, a: string): void { pool.set(id, 'text_halign', a); },
    set_text_valign(id: number, a: string): void { pool.set(id, 'text_valign', a); },
    set_text_formatting(id: number, fmt: unknown): void { pool.set(id, 'text_formatting', fmt); },
    set_text_wrap(id: number, wrap: string): void { pool.set(id, 'text_wrap', wrap); },
    ...
    set_xloc(id: number, left: number, right: number, xloc: string): void {
      pool.set(id, 'left', left); pool.set(id, 'right', right); pool.set(id, 'xloc', xloc);
    },
    get_left(id: number): unknown { return pool.get(id, 'left'); },
    get_top(id: number): unknown { return pool.get(id, 'top'); },
    get_right(id: number): unknown { return pool.get(id, 'right'); },
    get_bottom(id: number): unknown { return pool.get(id, 'bottom'); },
    delete(id: number): void { pool.remove(id); },
  };
}
...
export function makePolylineNs(pool: DrawingPool) {
  return {
    get all() { return allOf(pool, 'polyline'); },
...
export function makeLinefillNs(pool: DrawingPool) {
  return {
    get all() { return allOf(pool, 'linefill'); },
    new(line1: number, line2: number, color: string): number {
      return pool.create('linefill', drop({ line1, line2, color }));
    },
    set_color(id: number, c: string): void { pool.set(id, 'color', c); },
    get_line1(id: number): unknown { return pool.get(id, 'line1'); },
    get_line2(id: number): unknown { return pool.get(id, 'line2'); },
    delete(id: number): void { pool.remove(id); },
  };
}
...
export function makeTableNs(pool: DrawingPool) {
  return {
    get all() { return allOf(pool, 'table'); },
    ...
    cell(id: number, column: number, row: number, text = '', opts: Opts = {}): void {
      const o = pool.objects.get(id);
      if (o) (o.props.cells as Record<string, unknown>)[`${column},${row}`] = drop({ text, ...opts });
    },
    ...
    cell_set_text(id: number, column: number, row: number, text: string): void { cellSet(pool, id, column, row, 'text', text); },
    cell_set_tooltip(id: number, column: number, row: number, tooltip: string): void { cellSet(pool, id, column, row, 'tooltip', tooltip); },
    cell_set_text_font_family(id: number, column: number, row: number, family: string): void { cellSet(pool, id, column, row, 'text_font_family', family); },
    cell_set_bgcolor(id: number, column: number, row: number, c: string): void { cellSet(pool, id, column, row, 'bgcolor', c); },
    cell_set_text_color(id: number, column: number, row: number, c: string): void { cellSet(pool, id, column, row, 'text_color', c); },
    cell_set_text_size(id: number, column: number, row: number, size: string): void { cellSet(pool, id, column, row, 'text_size', size); },
    cell_set_text_halign(id: number, column: number, row: number, a: string): void { cellSet(pool, id, column, row, 'text_halign', a); },
    cell_set_text_valign(id: number, column: number, row: number, a: string): void { cellSet(pool, id, column, row, 'text_valign', a); },
    cell_set_text_formatting(id: number, column: number, row: number, fmt: unknown): void { cellSet(pool, id, column, row, 'text_formatting', fmt); },
    cell_set_width(id: number, column: number, row: number, w: number): void { cellSet(pool, id, column, row, 'width', w); },
    cell_set_height(id: number, column: number, row: number, h: number): void { cellSet(pool, id, column, row, 'height', h); },
    ...
    merge_cells(id: number, startColumn: number, startRow: number, endColumn: number, endRow: number): void {
      const o = pool.objects.get(id);
      if (!o) return;
      ...
      start.merged = { start_column: startColumn, start_row: startRow, end_column: endColumn, end_row: endRow };
    },
    set_position(id: number, p: string): void { pool.set(id, 'position', p); },
    set_bgcolor(id: number, c: string): void { pool.set(id, 'bgcolor', c); },
    set_border_color(id: number, c: string): void { pool.set(id, 'border_color', c); },
    set_border_width(id: number, w: number): void { pool.set(id, 'border_width', w); },
    set_frame_color(id: number, c: string): void { pool.set(id, 'frame_color', c); },
    set_frame_width(id: number, w: number): void { pool.set(id, 'frame_width', w); },
    clear(id: number): void { const o = pool.objects.get(id); if (o) o.props.cells = {}; },
    delete(id: number): void { pool.remove(id); },
  };
}

/** Set one attribute on a table cell, creating the cell entry if absent. */
function cellSet(pool: DrawingPool, id: number, column: number, row: number, key: string, value: unknown): void {
  const o = pool.objects.get(id);
  if (!o) return;
  const cells = o.props.cells as Record<string, unknown>;
  const k = `${column},${row}`;
