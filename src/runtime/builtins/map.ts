...
export const MapNs = {
  new(): Map<unknown, unknown> { return new Map(); },
  ...
  get(m: Map<unknown, unknown>, key: unknown): unknown { return m.has(key) ? m.get(key) : NA; },
  contains(m: Map<unknown, unknown>, key: unknown): boolean { return m.has(key); },
  remove(m: Map<unknown, unknown>, key: unknown): unknown {
    const v = m.has(key) ? m.get(key) : NA;
    m.delete(key);
    return v;
  },
  keys(m: Map<unknown, unknown>): unknown[] { return [...m.keys()]; },
  values(m: Map<unknown, unknown>): unknown[] { return [...m.values()]; },
  size(m: Map<unknown, unknown>): number { return m.size; },
  clear(m: Map<unknown, unknown>): void { m.clear(); },
  copy(m: Map<unknown, unknown>): Map<unknown, unknown> { return new Map(m); },
