...
function makeBars(n: number) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const o = px + (rnd() - 0.5) * 2,
      cl = px + (rnd() - 0.5) * 2;
    const hi = Math.max(o, cl) + rnd() * 2,
      lo = Math.min(o, cl) - rnd() * 2;
    const vol = 1000 + Math.floor(rnd() * 5000),
      t = i * 3600000;
    ...
    it(name, async () => {
      ...
      for (const c of cmps) {
        ...
        if (d.maxRel >= 1e-6 || d.naMis !== 0) {
          throw new Error(
            `${name}/${c.title}: maxRel=${d.maxRel.toExponential(2)} naMis=${d.naMis}`,
          );
        }
      }
    });
...
describe.skipIf(!PineTS)(
  'parity vs PineTS — documented divergences (piner matches the manual)',
  () => {
    ...
    it('crossover: piner fires per the Pine spec (x[1]<=y[1] and x>y); PineTS misses it', async () => {
      ...
      const bars: Bar[] = [4, 4, 4, 9, 9].map((cl, i) => ({
        time: i * 60000,
        open: cl,
        high: cl,
        low: cl,
...
const NS_EXACT: [string, string][] = [
  ...
  [
    'array.stdev biased vs sample',
    'var a=array.from(2.0,4.0,6.0,8.0)\nplot(array.stdev(a),"1")\nplot(array.stdev(a,false),"2")',
  ],
  [
  ...
  [
    'array.mode (smallest on tie)',
    'var a=array.from(3.0,3.0,1.0,1.0,2.0)\nplot(array.mode(a),"v")',
  ],
  [
    'array.median/sum/avg',
  ...
  [
    'color.red/green/black/olive channels',
    'plot(color.r(color.red),"1")\nplot(color.g(color.green),"2")\nplot(color.b(color.black),"3")\nplot(color.r(color.olive),"4")',
  ],
  // timeframe
  [
    'timeframe.in_seconds/isintraday',
  ...
  [
    ...
    'plot(ta.percentile_linear_interpolation(close,20,75),"v")',
  ],
  // valuewhen with occurrence (nth prior true)
  [
    'ta.valuewhen occurrence 1 & 2',
    'plot(ta.valuewhen(close>open,close,1),"1")\nplot(ta.valuewhen(close>open,close,2),"2")',
  ],
  // bare auto-typed input
  [
    'input() bare (int/float/bool)',
...
describe.skipIf(!PineTS)(
  'parity vs PineTS — verified piner-correct (PineTS missing or wrong)',
  () => {
    ...
    it('barstate.isfirst / islastconfirmedhistory', async () => {
      ...
      expect(f.data[1]).toBe(0);
      expect(l.data[PB.length - 1]).toBe(1);
...
describe.skipIf(!PineTS)(
  'newly-completed builtins (PineTS lacks/differs — manual ground truth)',
  () => {
...
const AUDIT_EXACT: [string, string][] = [
  [
    'matrix.is_square / elements_count',
    'var m=matrix.new<float>(2,2,1.0)\nplot(matrix.is_square(m)?1.0:0.0,"1")\nplot(matrix.elements_count(m),"2")',
  ],
  [
    'matrix.is_identity / is_zero',
...
describe('coverage-audit completions — piner ground truth', () => {
  it('ta.max / ta.min are the running all-time high / low', async () => {
    const tb: Bar[] = [3, 7, 2, 9, 5].map((px, i) => ({
      time: i * 60000,
      open: px,
      high: px,
      low: px,
