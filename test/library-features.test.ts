...
`,
    ...
    const d = bars();
    const last = 39;
    expect(js.outputs.plots.get(0)!.data[last]).toBeCloseTo(
      Math.hypot(d[last].close, d[last].high),
      9,
    );
  });
...
describe('library import — error handling', () => {
  ...
  it('missing library (direct) names the identity (Req 2.8)', () => {
    const e = throwsCompile(() =>
      compile(HEAD + 'import u/nope/1 as n\nplot(n.f(close))\n', { libraries: [lib] }),
    );
  ...
  it('missing library (transitive) names both the missing and importing identity (Req 8.4)', () => {
    ...
    const e = throwsCompile(() =>
      compile(HEAD + 'import u/mid/1 as m\nplot(m.h(close))\n', { libraries: [mid] }),
    );
  ...
  it('version mismatch reports requested vs available (Req 3.5)', () => {
    const e = throwsCompile(() =>
      compile(HEAD + 'import u/lib/2 as l\nplot(l.f(close))\n', { libraries: [lib] }),
    );
  ...
  it('duplicate alias — neither import binds (Req 3.6)', () => {
    ...
    const e = throwsCompile(() =>
      compile(HEAD + 'import u/lib/1 as x\nimport u/other/1 as x\nplot(x.f(close))\n', {
        libraries: [lib, lib2],
      }),
    );
  ...
  it('alias shadowing a builtin namespace (Req 3.7)', () => {
    const e = throwsCompile(() =>
      compile(HEAD + 'import u/lib/1 as math\nplot(math.f(close))\n', { libraries: [lib] }),
    );
  ...
  it('unresolved symbol (Req 4.4)', () => {
    const e = throwsCompile(() =>
      compile(HEAD + 'import u/lib/1 as l\nplot(l.nope(close))\n', { libraries: [lib] }),
    );
  ...
  it('private symbol is not exported (Req 4.5)', () => {
    ...
    const e = throwsCompile(() =>
      compile(HEAD + 'import u/p/1 as p\nplot(p.hidden(close))\n', { libraries: [priv] }),
    );
  ...
  it('cyclic dependency graph is rejected in cycle order (Req 8.3)', () => {
    ...
    const e = throwsCompile(() =>
      compile(HEAD + 'import u/a/1 as a\nplot(a.fa(close))\n', { libraries: [a, b] }),
    );
  ...
  it('export constraint: plot() inside an exported function (Req 7.1)', () => {
    ...
    const e = throwsCompile(() =>
      compile(HEAD + 'import u/bad/1 as bad\nplot(bad.draw(close))\n', { libraries: [bad] }),
    );
  ...
  it('cross-module recursion is rejected (Req 5.3)', () => {
    ...
    const e = throwsCompile(() =>
      compile(HEAD + 'import u/rec/1 as r\nplot(r.f(close))\n', { libraries: [selfrec] }),
    );
  ...
  it('imported script that is not a library is rejected', () => {
    ...
    const e = throwsCompile(() =>
      compile(HEAD + 'import u/ind/1 as x\nplot(x.f(close))\n', { libraries: [notlib] }),
    );
  ...
  it('library with a parse error is attributed to its identity (Req 9.1)', () => {
    ...
    let err: unknown;
    try {
      compile(HEAD + 'import u/broke/1 as b\nplot(b.f(close))\n', { libraries: [broken] });
    } catch (e) {
      err = e;
    }
  ...
  it('library title validation: non-string title (Req 1.7)', () => {
    const e = throwsCompile(() =>
      compile(HEAD + 'import u/t/1 as t\nplot(t.f(close))\n', {
  ...
  it('method dispatch with no matching arity is rejected (Req 4.8)', () => {
    ...
    const e = throwsCompile(() =>
      compile(HEAD + 'import u/mv/1 as mv\nv = mv.V.new(close)\nplot(v.scaleBy())\n', {
        libraries: [v],
      }),
    );
  ...
  it('missing required argument for an imported function is rejected (Req 5.5)', () => {
    ...
    const e = throwsCompile(() =>
      compile(HEAD + 'import u/two/1 as t\nplot(t.add(close))\n', { libraries: [two] }),
    );
  ...
  it('transitive import depth beyond 32 is rejected (Req 8.1, 8.5)', () => {
    ...
    const e = throwsCompile(() =>
      compile(HEAD + 'import u/L0/1 as l\nplot(l.f(close))\n', { libraries: libs }),
    );
  ...
  it('a transitive export-constraint violation carries the import chain (Req 9.4)', () => {
    ...
    const e = throwsCompile(() =>
      compile(HEAD + 'import u/midc/1 as m\nplot(m.go(close))\n', { libraries: [deep, mid] }),
    );
...
describe('library import — validation & scoping (audit fixes, medium)', () => {
  ...
  it('rejects duplicate exported function names (no user function overloading)', () => {
    ...
    const e = throwsCompile(() =>
      compile(HEAD + 'import u/dup/1 as d\nplot(d.pick(5))\n', { libraries: [dup] }),
    );
  ...
  it('rejects a library import whose alias shadows a builtin namespace', () => {
    ...
    const e = throwsCompile(() =>
      compile(HEAD + 'import u/outer/1 as o\nplot(o.g(close))\n', { libraries: [inner, outer] }),
    );
    expect(e.message).toContain('math');
  ...
  it('rejects duplicate import aliases inside a library', () => {
    ...
    const e = throwsCompile(() =>
      compile(HEAD + 'import u/coll/1 as o\nplot(o.g(close))\n', { libraries: [a, b, outer] }),
    );
  ...
  it('a private (non-exported) type used as an annotation is rejected', () => {
    ...
    const e = throwsCompile(() =>
      compile(HEAD + 'import u/priv/1 as p\np.Secret s = na\nplot(close)\n', { libraries: [lib] }),
    );
  ...
  it('a top-level consumer name colliding with an import alias is rejected', () => {
    ...
    const e = throwsCompile(() =>
      compile(HEAD + 'import u/m/1 as m\nm = 5.0\nplot(m.f(close))\n', { libraries: [lib] }),
    );
