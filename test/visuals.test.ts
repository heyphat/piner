...
const bars: Bar[] = Array.from({ length: 10 }, (_, i) => ({
  time: i * 60000, open: 100 + i, high: 120 + i, low: 80 + i, close: 100 + i * 2, volume: 1000,
}));
const HEX = /^#[0-9A-F]{8}$/;
...
function serializeVisuals(out: Engine['outputs']) {
  ...
  return JSON.stringify({
    plots: m(out.plots), markers: m(out.markers), candles: m(out.candles),
    hlines: m(out.hlines), fills: m(out.fills),
    barColors: [...out.barColors.entries()], bgColors: [...out.bgColors.entries()],
  });
}
...
describe('plotcandle', () => {
  it('records per-bar OHLC + color', async () => {
    ...
    expect(cs.data[5]).toEqual({ open: bars[5].open, high: bars[5].high, low: bars[5].low, close: bars[5].close });
