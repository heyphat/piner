...
import { compile, CompileError, Engine, ArrayFeed, type Bar, type LibraryRegistry, type CompiledScript } from '../src/index.js';

function bars(n = 10): Bar[] {
  return Array.from({ length: n }, (_, i) => ({ time: i * 60000, open: 10, high: 12, low: 8, close: 10, volume: 1 }));
}
...
describe('library audit fixes', () => {
  it('method overloads distinguished by container element type are NOT a duplicate export', () => {
    const reg: LibraryRegistry = [{ key: 'a/arrlib/1', source: `//@version=6
library("arrlib")
...
` }];
    const err = throwsCompile(() => compile(`//@version=6
indicator("c")
import a/arrlib/1 as al
plot(1)
`, { libraries: reg }));
  ...
  it('a builtin call (array.get) is not misattributed to a private fn that shares its name', () => {
    const reg: LibraryRegistry = [{ key: 'u/lib/1', source: `//@version=6
library("lib")
get(float x) => x * 2.0
...
plot(v.mag())
`, { libraries: reg });
    const js = await bothBackends(c, bars().map((b) => ({ ...b, close: 3 })));
  ...
  it('a namespaced method overload resolves when the receiver arg is itself a call', async () => {
    const reg: LibraryRegistry = [{ key: 'u/shape/1', source: `//@version=6
library("shape")
export type Circle
    float r = 0.0
export type Square
...
const POINT_LIB = { key: 'u/pointlib/1', source: `//@version=6
library("pointlib")
export type Point
    float x = 0.0
    float y = 0.0
` };
const MAG_LIB = { key: 'u/mathlib/1', source: `//@version=6
library("mathlib")
import u/pointlib/1 as t
export method mag(t.Point p) => p.x * p.x + p.y * p.y
` };
const MAG2_LIB = { key: 'u/mathlib2/1', source: `//@version=6
library("mathlib2")
import u/pointlib/1 as t
...
`;
  ...
  it("a library's own method on an imported type dispatches inside its own body", async () => {
    const lib3 = { key: 'u/mathlib3/1', source: `//@version=6
library("mathlib3")
import u/pointlib/1 as t
export method mag(t.Point p) => p.x * p.x + p.y * p.y
